...
class DeformableVisTRsegm3(nn.Module):
    def __init__(self, vistr: DeformableVisTR, only_positive_matches: bool, matcher: HungarianMatcher,
                 ...
                 post_processor: AllPostProcessor, top_k_inference: Union[int, None], mask_aux_loss: list, mask_attn_alignment: bool, use_box_coords: bool):
        ...
        self.use_box_coords = use_box_coords
        self.mask_embd_attention = DeformableAttSegmHead(hidden_dim, self.vistr.num_feature_levels, self.vistr.num_frames, mask_attn_alignment)
    ...
    def _get_training_embeddings(self, out, targets, hs, query_embed, reference_points, lvl):
        ...
        if not self.only_positive:
            ...
            for idx in embd_matched_idx:
                ...
                matched_ref_points.append(reference_points[0, idx])
    ...
    def _module_inference(self, hs_f, query_embed, memory_flatten, memories_att_map_f, masks_att_map, mask_head_feats_f, num_trajectories,
        ...
        hs_f = self.mask_embd_attention(memory_flatten, hs_f, query_embed, spatial_shapes, reference_points, valid_ratios, level_start_index)
    ...
    def forward(self, samples: NestedTensor, targets: dict):
        ...
        if self.training:
            ...
            for loss_lvl in loss_levels:
                if self.use_box_coords:
                    if loss_lvl == -1:
                        reference_points = out["pred_boxes"]
                    else:
                        reference_points = out["aux_outputs"][loss_lvl]["pred_boxes"]
                else:
                    if loss_lvl == 0:
                        ...
                        reference_points = inter_references[loss_lvl - 1]

                out_lvl = out if loss_lvl == -1 else out["aux_outputs"][loss_lvl]
                hs_f, query_embed_f, reference_points_f, num_trajectories = self._get_training_embeddings(out_lvl, targets, hs, query_pos, reference_points, loss_lvl)

                outputs_seg_masks = self._module_inference(hs_f, query_embed_f, memories_flatten, memories_att_map_f, masks_att_map, mask_head_feats_f,
            ...
            out["center_points"] = inter_references[-1]
            ...
            if self.top_k_inference is not None:
                ...
                outputs_seg_masks = self._module_inference(hs_f, query_pos, memories_flatten, memories_att_map_f, masks_att_map, mask_head_feats_f,
...
class MaskHeadConv(nn.Module):
    ...
    def __init__(self, dim, fpn_dims, nheads, use_deformable_conv, multi_scale_att_maps):
        ...
        self.out_lay1 = conv_layer(out_dims[i + 1], out_dims[i + 2], 3, padding=1)
        self.out_gn1 = torch.nn.GroupNorm(8, out_dims[i + 2])

        self.out_lay2 = conv_layer(out_dims[i + 2], 1, 3, padding=1)
...
class DeformableAttSegmHead(nn.Module):
    def __init__(self, d_model, num_feature_levels, num_frames, mask_attn_alignment):
        super().__init__()
        self.dec_connect_all_embeddings = True
        self.dec_embedding_correlation_alignment = False
        self.dec_embedding_correlation = True
        self.num_frames = num_frames
        self.temporal_window = self.num_frames - 1
        self.mask_attn_alignment = mask_attn_alignment
    ...
    def forward(self, src, tgt_masks, query_pos, src_spatial_shapes, reference_points, src_valid_ratios, src_level_start_index):
        ...
        other_frames_temporal_offsets = DeformableTransformerDecoder.get_temporal_offset_from_same_instance_all_connect(reference_points, other_temporal_offsets,
                                                                                                src_spatial_shapes, src_valid_ratios, self.mask_attn_alignment)
