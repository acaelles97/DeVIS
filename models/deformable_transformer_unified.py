...
from .ops.modules import MSDeformAttn, TemporalDifferentModuleMSDeformAttn, TemporalDifferentModuleMSDeformAttnUnified
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, num_frames=6, nhead_enc=8, nhead_dec=8,
        ...
        encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                          dropout, activation, self.num_frames,
                                                          num_feature_levels, nhead_enc, enc_n_curr_points)
        ...
        decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                              dropout, activation, num_frames,
                                                              dec_temporal_window, num_feature_levels, nhead_dec, dec_n_curr_points, dec_embedding_correlation)

        self.decoder = DeformableTransformerDecoder(decoder_layer, num_decoder_layers, dec_embedding_correlation, dec_embedding_correlation_alignment, dec_connect_all_embeddings, num_frames,  dec_temporal_window, return_intermediate_dec, with_gradient)
    ...
    def _reset_parameters(self):
        ...
        for m in self.modules():
            if isinstance(m, (MSDeformAttn, TemporalDifferentModuleMSDeformAttn, TemporalDifferentModuleMSDeformAttnUnified)):
                m._reset_parameters()
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, targets=None):
        ...
        return hs, query_embed, memory, init_reference_out, inter_references_out, level_start_index, valid_ratios, spatial_shapes
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 ...
                 n_frames=6, n_levels=4, n_heads=8, n_curr_points=4):

        super().__init__()
        self.self_attn = TemporalDifferentModuleMSDeformAttnUnified(n_frames, d_model, n_levels, n_heads, n_curr_points, False, False)
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def __init__(self, encoder_layer, num_layers, enc_temporal_window, enc_connect_all_embeddings):
        super().__init__()
        self.layers = _get_clones(encoder_layer, num_layers)
        self.num_layers = num_layers
        self.enc_temporal_window = enc_temporal_window
    ...
    def generate_temporal_offsets_all_connect(input_spatial_shapes, valid_ratios, device):
        ...
        for lvl in range(input_spatial_shapes.shape[0]):
            ...
            for curr_frame in range(0, T_):
                temporal_frames = torch.tensor([[0, t / T_] for t in range(-curr_frame, T_ - curr_frame)], device=device)
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_frames=36, t_window=2, n_levels=4, n_heads=8, n_curr_points=4, dec_embedding_correlation=False):

        super().__init__()
        self.cross_attn = TemporalDifferentModuleMSDeformAttnUnified(n_frames, d_model, n_levels, n_heads, n_curr_points, True, dec_embedding_correlation)
    ...
    def forward(self, tgt, query_pos, reference_points, curr_frame_offsets, temporal_offsets, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        # self attention
        ...
        # cross attention
        offsets = (curr_frame_offsets, temporal_offsets)
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, query_pos), reference_points, offsets,
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, dec_embedding_correlation, dec_embedding_correlation_alignment, dec_connect_all_embeddings,
                 num_frames, t_window, return_intermediate=False, with_gradient=False):
        super().__init__()
    ...
    def get_temporal_offset_from_same_instance_all_connect_parallel(reference_points, temporal_offsets, input_spatial_shapes, valid_ratios, dec_embedding_correlation_alignment):
        ...
        for curr_frame in range(0, T_):
            temporal_frames = curr_frame + torch.tensor([t for t in range(-curr_frame, T_ - curr_frame)], device=reference_points.device)
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        if self.dec_connect_all_embeddings:
            ...
            curr_frame_offsets, temporal_offset = self.get_curr_frame_and_temporal_offsets_decoder_all_connect_parallel(src_spatial_shapes,
                                                                                                                    embds_per_frame,
        ...
        other_frames_temporal_offsets =  torch.clone(temporal_offset)

        for lid, layer in enumerate(self.layers):
            if self.dec_embedding_correlation:
                if self.dec_connect_all_embeddings:
                    other_frames_temporal_offsets = self.get_temporal_offset_from_same_instance_all_connect_parallel(reference_points, temporal_offset, src_spatial_shapes, src_valid_ratios, self.dec_embedding_correlation_alignment)

                else:
                    raise NotImplementedError
                    # other_frames_temporal_offsets = self.get_temporal_offset_from_same_instance_parallel(reference_points, self.temporal_window, other_temporal_offsets, src_spatial_shapes, src_valid_ratios, self.dec_embedding_correlation_alignment)
            ...
            output = layer(output, query_pos, reference_points_input, curr_frame_offsets, other_frames_temporal_offsets, src, src_spatial_shapes, src_level_start_index, src_padding_mask)
...
def build_deforamble_unified_transformer(args):
