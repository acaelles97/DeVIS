...
from models.ops.modules import MSDeformAttn, TemporalMSDeformAttnEncoder, TemporalMSDeformAttnDecoder
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 num_feature_levels=4, dec_n_points=4,  enc_n_points=4, with_gradient=False, init_modules=True):
    ...
    def _reset_parameters(self):
        ...
        for m in self.modules():
            if isinstance(m, (MSDeformAttn, TemporalMSDeformAttnEncoder, TemporalMSDeformAttnDecoder)):
                m._reset_parameters()
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, **kwargs):
        # self attention
        src2 = self.self_attn(self.with_pos_embed(src, pos), reference_points, src, spatial_shapes, level_start_index, **kwargs)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, **kwargs):
        # self attention
        ...
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, query_pos),
                               reference_points, src, src_spatial_shapes, level_start_index,  **kwargs)
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, with_gradient=True, return_intermediate=False):
        super().__init__()
    ...
    def refine_reference_point(self, lid, output, reference_points, intermediate, intermediate_reference_points):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        kwargs['src_padding_mask'] = src_padding_mask
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output, query_pos, reference_points_input, src, src_spatial_shapes, src_level_start_index, **kwargs)
...
def build_deformable_transformer(cfg):
    return DeformableTransformer(
        ...
        nhead=cfg.TRANSFORMER.N_HEADS,
        dim_feedforward=cfg.MODEL.DIM_FEEDFORWARD,
        dropout=cfg.MODEL.DROPOUT,
        num_feature_levels=cfg.MODEL.NUM_FEATURE_LEVELS,
        num_encoder_layers=cfg.TRANSFORMER.ENCODER_LAYERS,
        num_decoder_layers=cfg.TRANSFORMER.DECODER_LAYERS,
        dec_n_points=cfg.MODEL.TRANSFORMER.ENC_N_POINTS,
        enc_n_points=cfg.TRANSFORMER.DEC_N_POINTS,
        activation="relu",
