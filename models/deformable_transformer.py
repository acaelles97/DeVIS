...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, targets=None):
        ...
        return hs, query_embed, memory, init_reference_out, inter_references_out, level_start_index, valid_ratios, spatial_shapes, None
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        super().__init__()
        self.cross_attn = TemporalDifferentModuleMSDeformAttn(n_frames, d_model, n_levels, t_window, n_heads, n_curr_points, n_temporal_points, False, True, dec_embedding_correlation)
...
class DeformableTransformerDecoderLayerExtraSelfAttn(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_frames=36, t_window=2, n_levels=4, n_heads=8, n_curr_points=4, n_temporal_points=2, dec_embedding_correlation=False,
                 with_decoder_instance_self_attn=False, with_decoder_frame_self_attn=False):

        super().__init__()
        assert with_decoder_frame_self_attn or with_decoder_instance_self_attn
        self.with_decoder_frame_self_attn = with_decoder_frame_self_attn
        self.with_decoder_instance_self_attn = with_decoder_instance_self_attn
        ...
        self.cross_attn = TemporalDifferentModuleMSDeformAttn(n_frames, d_model, n_levels, t_window, n_heads, n_curr_points, n_temporal_points,
                                                              True, dec_embedding_correlation, False)
        ...
        # self attention
        self.self_attn_inst = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout2_inst = nn.Dropout(dropout)
        self.norm2_inst = nn.LayerNorm(d_model)
    ...
    def forward(self, tgt, query_pos, reference_points, curr_frame_offsets, other_frames_temporal_offsets, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        ...
        if self.with_decoder_instance_self_attn:
            tgt2 = self.self_attn_inst(q, k, tgt)[0]
        else:
            tgt2 = self.self_attn_inst(q.transpose(0, 1), k.transpose(0, 1), tgt.transpose(0, 1))[0]
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output, query_pos, reference_points_input, curr_temporal_offset, other_frames_temporal_offsets, src, src_spatial_shapes, src_level_start_index, src_padding_mask)
...
#                         if dec_embedding_correlation_alignment:
#                             frame_alignments.append(reference_points_per_frame[timestep_used, :, 2:] / distance_normalizer)
#                     else:
#                         assert reference_points.shape[-1] == 2
