...
def torch_prepare_heatmap(hm_h, hm_w, bbox):
    pre_hm = torch.zeros((1, hm_h, hm_w), dtype=torch.float32, device=bbox.device)
...
class DeformableVisTRsegm4(nn.Module):
    def __init__(self, vistr: DeformableVisTR, only_positive_matches: bool, matcher: HungarianMatcher,
                 ...
                 post_processor: AllPostProcessor, top_k_inference: Union[int, None], mask_aux_loss: list, use_ct_distance: bool):

        super().__init__()
        self.vistr = vistr
        self.only_positive = only_positive_matches
        self.top_k_inference = top_k_inference
        self.use_ct_distance = use_ct_distance
    ...
    def _get_training_embeddings(self, out, targets, hs, boxes, lvl):
        ...
        if not self.only_positive:
            ...
            for idx in embd_matched_idx:
                ...
                matched_boxes.append(boxes[0, idx])
            hs_f = torch.stack(matched_hs, dim=1)
            # boxes_f = torch.cat(matched_boxes, dim=0)
            boxes_f = torch.stack(matched_boxes, dim=1)
            out["indices"] = indices
            out["mask_valid_ind"] = valid_ind
            out["original_valid_ordered_ind"] = original_valid_ordered_ind
        return hs_f, boxes_f, num_trajectories

    def _get_eval_top_k_embeddings(self, out, box, targets, hs, inter_references):
        ...
        boxes_f = box[:, top_k_embeddings]
        return  hs_f, boxes_f, num_trajectories, results, inverse_idxs


    def _module_inference(self, hs_f, hp_f, memories_att_map_f, masks_att_map, mask_head_feats_f, num_trajectories):
        ...
        seg_masks_f = self.mask_head(mask_head_feats_f, hp_f, bbox_mask_flattened, instances_per_batch=num_trajectories)
        outputs_seg_masks = seg_masks_f.view((num_trajectories, self.vistr.num_frames) + seg_masks_f.shape[2:])
        return outputs_seg_masks

    def compute_heatmap(self, boxes, spatial_shape):
        boxes = box_cxcywh_to_xyxy(boxes)
        img_h, img_w = torch.tensor([spatial_shape], device=boxes.device).unbind(-1)
        ...
        for i in range(boxes.shape[0]):
            instance_hp.append(torch_prepare_heatmap(spatial_shape[0], spatial_shape[1], boxes[i]))

        instance_hp = torch.stack(instance_hp, dim=0)
        return instance_hp
        ...
        if self.training:
            ...
            for loss_lvl in loss_levels:
                if loss_lvl == -1:
                    ...
                    boxes = out["aux_outputs"][loss_lvl]["pred_boxes"]

                out_lvl = out if loss_lvl == -1 else out["aux_outputs"][loss_lvl]
                hs_f, boxes_f, num_trajectories = self._get_training_embeddings(out_lvl, targets, hs, boxes, loss_lvl)
                higher_res = mask_head_feats_f[-1].shape[-2:]
                if self.use_ct_distance:
                    ct = boxes_f[:, :, :2]
                    ct = compute_centroid_feature_map(higher_res, ct)
                    hp_f = ct.transpose(0, 1).flatten(0, 1)
                else:
                    boxes_f = boxes_f.transpose(0, 1).flatten(0, 1)
                    hp_f = self.compute_heatmap(boxes_f, higher_res)

                outputs_seg_masks = self._module_inference(hs_f, hp_f, memories_att_map_f, masks_att_map, mask_head_feats_f, num_trajectories)
            ...
            if "softmax_prediction" not in targets.keys():
                ...
                box_f = out["pred_boxes"][0].view(self.vistr.num_frames, num_trajectories,  out["pred_boxes"].shape[-1])
                inter_references = inter_references[-1][0].view(self.vistr.num_frames, num_trajectories, inter_references.shape[-1])
                hs_f, boxes_f, num_trajectories, results, inverse_idxs = self._get_eval_top_k_embeddings(out, box_f, targets, hs_f, inter_references)
                higher_res = mask_head_feats_f[-1].shape[-2:]
                if self.use_ct_distance:
                    ct = boxes_f[:, :, :2]
                    ct = compute_centroid_feature_map(higher_res, ct)
                ...
                num_trajectories = self.vistr.num_queries if self.vistr.use_trajectory_queries else self.vistr.num_queries // self.vistr.num_frames
                results = postprocess_softmax(self.vistr.num_frames, out, targets["tgt_size"], targets["clip_length"])
                hs_f = hs[-1][0].view(self.vistr.num_frames, num_trajectories, hs.shape[-1])
                box_f = out["pred_boxes"][0].view(self.vistr.num_frames, num_trajectories,  out["pred_boxes"].shape[-1])
                inter_references = inter_references[-1][0].view(self.vistr.num_frames, num_trajectories, inter_references.shape[-1])
                higher_res = mask_head_feats_f[-1].shape[-2:]
                # hp_f = torch.zeros(360, higher_res[0], higher_res[1], device=box_f.device)
                if self.use_ct_distance:
                    ct = box_f[:, :, :2]
                    ct = compute_centroid_feature_map(higher_res, ct)
                    hp_f = ct.transpose(0, 1).flatten(0, 1)
                else:
                    boxes_f = box_f.transpose(0, 1).flatten(0, 1)
                    hp_f = self.compute_heatmap(boxes_f, higher_res)

                outputs_seg_masks = self._module_inference(hs_f, hp_f, memories_att_map_f, masks_att_map, mask_head_feats_f, num_trajectories)
                out_masks = outputs_seg_masks.transpose(0, 1)
                results["masks"] = out_masks
                if inter_references.shape[-1] == 4:
                    results["centroid_points"] = inter_references[:, :, :2]
                else:
                    results["centroid_points"] = inter_references
...
class MaskHeadConv(nn.Module):
    ...
    def __init__(self, dim, fpn_dims, nheads, use_deformable_conv, multi_scale_att_maps):
        ...
        for i in range(1, num_levels):
            if i == num_levels -1:
                setattr(self, f"lay{i + 2}", conv_layer(in_dims[i]+1, out_dims[i + 1], 3, padding=1))
