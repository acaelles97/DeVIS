...
from models.ops.modules import TemporalMSDeformAttnEncoder, TemporalMSDeformAttnDecoder
...
class DeVISTransformer(DeformableTransformer):
    def __init__(self, d_model=256, num_frames=6, nhead=8, num_encoder_layers=6, num_decoder_layers=6, dim_feedforward=1024, dropout=0.1, activation="relu",
        ...
        super().__init__(d_model=d_model, nhead=nhead, num_feature_levels=num_feature_levels, with_gradient=with_gradient, init_modules=False)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        torch.save(memory, "/usr/stud/cad/results/trainings/debug/out_to_check/memory_new.pth")
        ...
        # torch.save(hs, "/usr/stud/cad/results/trainings/debug/out_to_check/hs_new.pth")
        # torch.save(inter_references, "/usr/stud/cad/results/trainings/debug/out_to_check/inter_references_new.pth")
...
class DeVISTransformerEncoder(DeformableTransformerEncoder):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        T_ = src.shape[0] if self.enc_connect_all_embeddings else self.t_window
...
class DeVISTransformerDecoder(DeformableTransformerDecoder):
    def __init__(self, decoder_layer, num_layers, with_gradient=False, instance_aware_att=True):
        super().__init__(decoder_layer=decoder_layer, num_layers=num_layers, with_gradient=with_gradient, return_intermediate=True)
