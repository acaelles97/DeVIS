...
class DeformableVisTRsegm2(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets: dict):
        # out, backbone_feats, memories, hs, srcs, masks = self.vistr(samples, targets)
...
class MaskHeadConv(nn.Module):
    ...
    def __init__(self, dim, fpn_dims, nheads, use_deformable_conv, multi_scale_att_maps, use_out_mdcn=False):
        super().__init__()
