...
import json
from util.att_maps_viz import visualize_clips_with_att_maps, plot_both_masks, create_masks, visualize_clips_with_att_maps_merged_res
...
class Tracker:
    ...
    def parse_temporal_points(self, vid_info, tracks, top_k_idx, single_res=None, layer=5):

        use_single_res = single_res is not None
        spatial_shapes = torch.stack([vid_info['src_spatial_shapes'][..., 2], vid_info['src_spatial_shapes'][..., 1]], -1)

        num_queries = vid_info["other_temporal_offsets"].shape[0]
        ref_point = vid_info[f"ref_point_{layer}"].view((6, num_queries // 6) + (vid_info[f"ref_point_{layer}"].shape[-1], ))[:, top_k_idx]

        curr_temporal_offset = vid_info["curr_temporal_offset"]
        curr_temporal_offset = curr_temporal_offset.view((6, num_queries // 6) + curr_temporal_offset.shape[1:])
        curr_temporal_offset = curr_temporal_offset[:, top_k_idx]

        temporal_attention_weights_temporal = vid_info[f"temporal_attention_weights_temporal_{layer}"]
        attention_weights_curr = vid_info[f"attention_weights_curr_{layer}"]
        sampling_temporal_offsets = vid_info[f'sampling_temporal_offsets_{layer}']
        current_sampling_offsets = vid_info[f'current_sampling_offsets_{layer}']

        temporal_attention_weights_temporal = temporal_attention_weights_temporal.view(
            (6, num_queries // 6) + temporal_attention_weights_temporal.shape[2:])
        temporal_attention_weights_temporal = temporal_attention_weights_temporal[:, top_k_idx]

        attention_weights_curr = attention_weights_curr.view((6, num_queries // 6) + attention_weights_curr.shape[2:])
        attention_weights_curr = attention_weights_curr[:, top_k_idx]

        sampling_temporal_offsets = sampling_temporal_offsets.view((6, num_queries // 6) + sampling_temporal_offsets.shape[2:])
        sampling_temporal_offsets = sampling_temporal_offsets[:, top_k_idx]

        current_sampling_offsets = current_sampling_offsets.view((6, num_queries // 6) + current_sampling_offsets.shape[2:])
        current_sampling_offsets = current_sampling_offsets[:, top_k_idx]

        real_temporal_offsets = []
        for curr_frame in range(0, 6):
            temporal_frames = torch.tensor([[0, curr_frame / 6]], device=sampling_temporal_offsets.device) + torch.tensor(
                [[0, t / 6] for t in range(-curr_frame, 6 - curr_frame) if t != 0], device=sampling_temporal_offsets.device)
            real_temporal_offsets.append(temporal_frames)

        real_temporal_offsets = torch.stack(real_temporal_offsets, dim=0)

        temporal_positions = (sampling_temporal_offsets - real_temporal_offsets[:, None, None, None, :, None]) / torch.tensor([[1, 1 / 6]],
                                                                                                                              device=sampling_temporal_offsets.device)

        curr_positions = (current_sampling_offsets - curr_temporal_offset[:, :, None, :, None]) / torch.tensor([[1, 1 / 6]],
                                                                                                               device=sampling_temporal_offsets.device)
        if use_single_res:
            tgt_res = spatial_shapes[single_res]
            curr_positions = curr_positions * tgt_res
            temporal_positions = temporal_positions * tgt_res

        else:
            curr_positions = curr_positions * spatial_shapes[None, None, None, :, None, ]
            temporal_positions = temporal_positions * spatial_shapes[None, None, None, :, None, None, ]

        for i, track in enumerate(tracks):
            track.curr_position = torch.round(curr_positions[:, i]).type(torch.long)
            track.curr_att_weights = attention_weights_curr[:, i]

            track.temporal_positions = torch.round(temporal_positions[:, i]).type(torch.long)
            track.temporal_att_weights = temporal_attention_weights_temporal[:, i]
            track.spatial_shapes = spatial_shapes
            track.ref_point = ref_point[:, i]
    ...
    def __call__(self, video, device, args):
        ...
        for idx, video_clip in enumerate(video_loader):
            ...
            print(f"Top k {results['top_k_idxs']}")
            if results['top_k_idxs'] != 13:
                continue
            ...
            self.parse_temporal_points(results, clip_tracks, results["top_k_idxs"], use_single_res, layer_used)
            ...
            # plot_both_masks(idx, video.images_folder, video.video_clips[idx][:clip_length], clips_to_show, out_path=args.out_viz_path, class_name=cat_names)

            visualize_clips_with_att_maps(idx, video.images_folder, video.video_clips[idx][:clip_length], clips_to_show, layer_used, out_path=args.out_viz_path, class_name=cat_names)

            # visualize_clips_with_att_maps_merged_res(idx, video.images_folder, video.video_clips[idx][:clip_length], clips_to_show, use_single_res,
            #                                          out_path=args.out_viz_path, class_name=cat_names)

            # create_masks(idx, video.images_folder, video.video_clips[idx][:clip_length], clips_to_show, out_path=args.out_viz_path, class_name=cat_names)
...
def run_inference(model, data_loader_val, device, args):
    ...
    # video_names = ["c8518f8355"]
    # video_names = ["98d4c5908c", "ad3ff1fb2e", "bfb346a33b", "c35af243fd"]

    video_names = ["f6ed698261", "4348676053"]
