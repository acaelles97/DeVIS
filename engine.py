...
import trackeval
from datasets.vis import VISValDataset
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, epoch)):
        # if not targets[0]['valid'].sum() == 0:
        #     continue
        samples = samples.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        # if torch.all(targets[0]["valid"]):
        #     continue
        # if torch.sum(targets[0]["valid"]) < 12:
        #     continue

        # if (targets[0]["valid"].shape[0] <= args.num_frames):
        #     continue
        # torch.save(samples, "/usr/stud/cad/results/trainings/debug/out_to_check/samples_new.pth")
        # torch.save(targets, "/usr/stud/cad/results/trainings/debug/out_to_check/targets_new.pth")
        # torch.save(model.module.state_dict(), "/usr/stud/cad/results/trainings/debug/out_to_check/weights_new.pth")
...
@torch.no_grad()
def inference_vis(tracker: Tracker, data_loader_val: DataLoader, dataset_val: VISValDataset, visualizers: dict, device: torch.device, output_dir: Path, out_folder_name: str, epoch: int):
    tracker.model.eval()
