...
def visualize_clips_with_att_maps(idx, folder_images, images_path, tracks, layer, out_path, class_name, ):
    video_name = images_path[0].split("/")[0]
    cmap = create_color_map()[1:]
    ...
    for track in tracks:
        ...
        for lvl in range(track.spatial_shapes.shape[0]):
            ...
            for i, curr_frame in enumerate(range(num_frames)):
                ...
                for idx, temporal_frame in enumerate(temporal_frames):
                    ...
                    feature_map = torch.zeros((att_map_resolution[1], att_map_resolution[0]), device=track.temporal_positions.device)

                    if temporal_frame == 0:
                        curr_frame_positions = track.curr_position[curr_frame, :, lvl].flatten(0, 1)
                        curr_frame_positions[:, 0] = torch.clamp(curr_frame_positions[:, 0], 0, att_map_resolution[0] - 1)
                        curr_frame_positions[:, 1] = torch.clamp(curr_frame_positions[:, 1], 0, att_map_resolution[1] - 1)
                        curr_frame_weights = track.curr_att_weights[curr_frame, :, lvl].flatten(0, 1)
                        ...
                        feature_map = F.interpolate(feature_map[None, None], image_shape, mode="bilinear", align_corners=False).detach().to("cpu")[0, 0]
                        ...
                        if ref_point.shape[-1] == 4:
                            ...
                            boxes = boxes.cpu() *  torch.tensor([image_shape[1], image_shape[0], image_shape[1], image_shape[0]])
                            ...
                            p = PatchCollection(
                                [Polygon(np_poly)], facecolor='none', edgecolors=[(1, 0, 0)], linewidths=2)
                            axs[i+1, idx+1].add_collection(p)

                        else:
                            centroid_ref_point = torch.round(ref_point * torch.tensor([image_shape[1], image_shape[0]], device=ref_point.device)).cpu()

                        mymarker = axs[i+1, idx+1].scatter(centroid_ref_point[0], centroid_ref_point[1], s=80, c='red', marker='x', linewidths=1.4)
                        axs[i+1, idx+1].add_artist(mymarker)
                        axs[i+1, idx+1].axis('off')

                    else:
                        temporal_positions = track.temporal_positions[curr_frame, :, lvl, temporal_counter].flatten(0, 1)
                        temporal_positions[:, 0] = torch.clamp(temporal_positions[:, 0], 0, att_map_resolution[0] - 1)
                        temporal_positions[:, 1] = torch.clamp(temporal_positions[:, 1], 0, att_map_resolution[1] - 1)
                        temporal_frame_weights = track.temporal_att_weights[curr_frame, :, lvl, temporal_counter].flatten(0, 1)

                        for position, weight in zip(temporal_positions, temporal_frame_weights):
                            feature_map[position[1], position[0]] += weight

                        feature_map = \
                        F.interpolate(feature_map[None, None], image_shape, mode="bilinear", align_corners=False).detach().to("cpu")[0, 0]
                        # feature_map = feature_map.to("cpu")
                        axs[i + 1, idx + 1].imshow(feature_map, cmap='cividis')
                        axs[i + 1, idx + 1].axis('off')
                        ...
                        if ref_point.shape[-1] == 4:
                            ...
                            p = PatchCollection(
                                [Polygon(np_poly)], facecolor='none', edgecolors=[(0.5, 1, 0)], linewidths=2)
                            axs[i+1, idx+1].add_collection(p)

                        else:
                            centroid_ref_point = torch.round(ref_point * torch.tensor([image_shape[1], image_shape[0]], device=ref_point.device)).cpu()

                        mymarker = axs[i+1, idx+1].scatter(centroid_ref_point[0], centroid_ref_point[1], s=80, c=(0.5, 1, 0), marker='x', linewidths=1.4)
                        axs[i+1, idx+1].add_artist(mymarker)
            ...
            plt.margins(0, 0)
            plt.savefig(os.path.join(out_folder_att_maps, f"res_{track.spatial_shapes[lvl][1]}-{ track.spatial_shapes[lvl][0]}.png"), bbox_inches='tight')
        ...
        for idx, image_path in enumerate(images_path):
            ...
            if track.masks[idx] is None:
                ...
                imshow_det_masks_for_tracks(image, instance, cmap=cmap, class_names=class_name, out_file=out_image_path)

    plt.close('all')


def visualize_clips_with_att_maps_merged_res(idx, folder_images, images_path, tracks, single_res, out_path, class_name):
    video_name = images_path[0].split("/")[0]
    cmap = create_color_map()[1:]
    ...
    for track in tracks:
        ...
        fig, axs = plt.subplots(ncols=num_frames+1, nrows=num_frames+1, figsize=(40, 30))
        axs[0, 0].axis('off')
        ...
        for i, t in enumerate(range(num_frames)):
            ...
            for id_, track_to_put in enumerate(tracks):
                ...
                p = PatchCollection(
                    [Polygon(np_poly)], facecolor='none', edgecolors=[(1, 0, 0)], linewidths=1)
        ...
        for i, curr_frame in enumerate(range(num_frames)):
            ...
            for idx, temporal_frame in enumerate(temporal_frames):
                res_lvl = [1, 2, 3]
                att_maps = torch.zeros((image_shape[0], image_shape[1]))
                ref_point_circle = None

                for lvl in range(track.spatial_shapes.shape[0]):
                    if lvl not in res_lvl:
                        continue
                    if use_single_res:
                        att_map_resolution = track.spatial_shapes[single_res]
                    else:
                        att_map_resolution = track.spatial_shapes[lvl]

                    feature_map_lvl = torch.zeros((att_map_resolution[1], att_map_resolution[0]), device=track.temporal_positions.device)
                    ...
                    if temporal_frame == 0:
                        ...
                        for position, weight in zip(curr_frame_positions, curr_frame_weights):
                            feature_map_lvl[position[1], position[0]] += weight

                        feature_map_lvl = F.interpolate(feature_map_lvl[None, None], image_shape, mode="bilinear", align_corners=False).detach().to("cpu")[0, 0]
                        ref_point =  track.boxes[curr_frame]
                        ref_point = np.array([(ref_point[0] + ref_point[2]) / 2, (ref_point[1] + ref_point[3]) / 2,]).astype(np.int32)
                        ref_point_circle = Circle((ref_point[0], ref_point[1]), radius=10, fill=True, color='r')
                        ...
                        ref_point =  track.boxes[curr_frame + temporal_frame]
                        ref_point = np.array([(ref_point[0] + ref_point[2]) / 2, (ref_point[1] + ref_point[3]) / 2,]).astype(np.int32)
                        ref_point_circle = Circle((ref_point[0], ref_point[1]), radius=10, fill=True, color=(0.5, 1, 0))


                    feature_map_lvl = (feature_map_lvl / feature_map_lvl.max())
                    att_maps += feature_map_lvl

                axs[i + 1, idx + 1].imshow(att_maps, cmap='cividis')
                if ref_point_circle is not None:
                    axs[i + 1, idx + 1].add_patch(ref_point_circle)
                axs[i + 1, idx + 1].axis('off')
        ...
        plt.savefig(os.path.join(out_folder_att_maps, "out_att_maps.png"), bbox_inches='tight')
