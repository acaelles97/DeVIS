...
def overlay_mask_on_image(image, mask, mask_opacity=0.8, mask_color=(0, 255, 0)):
    if mask.ndim == 3:
...
def parse_results_per_frame(video_results, num_frames):
    gathered_instances = [[] for _ in range(num_frames)]

    for idx, result in enumerate(video_results):
        for t in range(len(result["segmentations"])):
            frame_result = {
                "instance_id": idx,
                "mask": result["segmentations"][t],
                "score": result["score"],
                "category_id": result["category_id"],
            }
            gathered_instances[t].append(frame_result)

    return gathered_instances

def visualize_raw_detections(folder_images, images_path, clip_detections, clip_id, out_path, class_name, min_score=0.2, ):
    video_name = images_path[0].split("/")[0]
    cmap = create_color_map()
    out_folder = os.path.join(out_path, video_name, "raw_detections")
    os.makedirs(out_folder, exist_ok=True)
    for idx, (image_path, frame_detections) in enumerate(zip(images_path, clip_detections)):
        frame_name = image_path.split("/")[1].split(".")[0]
        image = cv2.imread(os.path.join(folder_images, image_path))
        out_folder_image = os.path.join(out_folder, frame_name)
        os.makedirs(out_folder_image, exist_ok=True)
        for instance in frame_detections:
            image_to_overlay = np.copy(image)
            if instance["mask"] is None or instance["score"] < min_score:
                continue
            # mask = mask_tools.decode(instance["mask"])
            # image_to_save = overlay_mask_on_image(image_to_overlay, mask == 1, mask_color=cmap[instance["instance_id"]], mask_opacity=0.8)
            out_image_path = os.path.join(out_folder_image, f"{clip_id}_{instance['instance_id']}.jpg")
            # cv2.imwrite(out_image_path, image_to_save)
            # frame_results = {
            #     "mask": instance["mask"][idx],
            #     "score": instance["score"],
            #     "category_id": instance["category_id"],
            #     "instance_id": instance,
            # }
            imshow_det_bboxes_for_tracks(image_to_overlay, instance, cmap=cmap, class_names=class_name, out_file=out_image_path)
