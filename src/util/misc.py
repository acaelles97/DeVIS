...
# Makes sure that we do not provide repeated predictions from same videos coming from different processes
def accumulate_results(results_accums):
    done_vides = set()
    gathered_preds = []
    for process_result in results_accums:
        procces_videos_ids = list(set([res["video_id"] for res in process_result]))
        ids_to_gather = [video_id for video_id in procces_videos_ids if video_id not in done_vides]
        done_vides.update(procces_videos_ids)
        for pred in process_result:
            if pred["video_id"] in ids_to_gather:
                gathered_preds.append(pred)
