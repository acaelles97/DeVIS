...
def visualize_clips_with_att_maps(idx, folder_images, images_path, tracks, layer, res_lvls, out_path, class_name, ):
    video_name = images_path[0].split("/")[0]
    cmap = create_color_map()[1:]
            ...
            for i, curr_frame in enumerate(range(num_frames)):
                ...
                for idx, temporal_frame in enumerate(temporal_frames):
                    ...
                    if temporal_frame == 0:
                        ...
                        curr_frame_positions[:, 0].clamp_(0, att_map_resolution[0] - 1)
                        curr_frame_positions[:, 1].clamp_(0, att_map_resolution[1] - 1)
                        ...
                        temporal_positions = track.temporal_positions[curr_frame, :, temporal_counter, lvl].flatten(0, 1)
                        temporal_positions[:, 0].clamp_(0, att_map_resolution[0] - 1)
                        temporal_positions[:, 1].clamp_(0, att_map_resolution[1] - 1)
                        temporal_frame_weights = track.temporal_att_weights[curr_frame, :, temporal_counter, lvl].flatten(0, 1)
