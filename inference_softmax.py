...
class Track:
    ...
    def process_masks(self, idx, t_window, tgt_size, encode_all_mask, num_masks, masks):
        processed_masks = []
    ...
    def update(self, idx, t_window, tgt_size,  scores, categories, boxes, masks, centroid_points, top_k, mask_id, encode_all_mask):
        ...
        if top_k and isinstance(masks, list):
            ...
            self.process_masks(idx, t_window, tgt_size, encode_all_mask, num_masks, masks)
    ...
    def __call__(self, video, device, args):
        ...
        for idx, video_clip in enumerate(video_loader):
            ...
            for i, track in enumerate(clip_tracks):
                # We need to maintain probabilities if we want
                encode_all_masks = self.hungarian_matcher.use_binary_mask_iou
                mask_id = i
                masks =  pred_masks[:, i]
                centroid_points = pred_centroid_points[:, i]

                track.update(idx, self.overlap_window, video.original_size, pred_scores[:, i], pred_classes[:, i], pred_boxes[:, i], masks,
                             centroid_points, None, mask_id, encode_all_masks)
            ...
            if video_tracks is None:
                ...
                for track in video_tracks:
                    track.update_curr_timestep(clip_length)
                ...
                for track in video_tracks:
                    track.update_curr_timestep(clip_length-self.overlap_window)
